...
export default function Skills() {
  ...
  return (
    ...
    >
      ...
      <motion.p className='mt-2 mb-16 text-white/90 text-base sm:text-lg z-10'
      ...
      <div className='relative w-full overflow-visible max-w-full' style={{ transformStyle: "preserve-3d" }}>
        ...
        >
          {repeated.map((s, i) => (
            <div key={`bg-${i}`} className='flex flex-col items-center gap-2 min-w-32'>
              <span>{s.icon}</span>
            </div>
          ))}
        </motion.div>
        ...
        >
          {repeated.map((s, i) => (
            <div
              key={`mid-${i}`} className='flex flex-col items-center gap-4 min-w-40'
              aria-label={s.name}
              title={s.name}
            >
        ...
        >
          {repeated.map((s, i) => (
            <div key={`fg-${i}`} className='flex flex-col items-center gap-2 min-w-32'>
              <span>{s.icon}</span>
            </div>
          ))}
        </motion.div>
